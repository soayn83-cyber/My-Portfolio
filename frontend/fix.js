...
if (i === -1) {
  ...
  console.log(content.indexOf('일러스트/스케치용'));
} else {
  const replacement = `            ) : formData.category === "work_process" ? (
              <div className="space-y-4 pt-4 border-t border-primary/10">
                ...
                <div className="flex gap-4 overflow-x-auto pb-4 pt-2 snap-x items-stretch">
                  {formData.work_steps && formData.work_steps.map((step, index) => (
                    <div key={index} className="min-w-[300px] w-[300px] flex-shrink-0 snap-center rounded-lg border border-primary/20 bg-muted/30 p-4 space-y-4 flex flex-col relative h-[450px]">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold text-lg text-primary">Step {index + 1}</span>
                        <Button type="button" variant="ghost" size="sm" className="text-red-500 hover:bg-red-500/10 hover:text-red-600 h-8 px-2" onClick={() => removeWorkStep(index)}>X</Button>
                      </div>
                      <div className="flex-shrink-0 h-[200px] w-full border border-primary/20 rounded-md relative overflow-hidden bg-background">
                        {step.image_url ? (
                          <div className="w-full h-full relative group">
                            <Image src={step.image_url} alt={\`Step \${index + 1}\`} fill className="object-contain" />
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <Label className="cursor-pointer text-white text-sm bg-black/50 px-3 py-1 rounded-md">
                                <input type="file" accept="image/*" className="hidden" onChange={(e) => handleWorkStepImageUpload(index, e)} />
                                변경
                              </Label>
                            </div>
                          </div>
                      ...
                      <div className="flex-1 min-h-0 pt-2">
                         <Textarea placeholder="이 단계의 과정을 설명해주세요." value={step.description || ""} onChange={(e) => handleWorkStepChange(index, e.target.value)} className="w-full h-full resize-none text-sm bg-background/50 border-primary/20 p-3" />
                      </div>
                    </div>
                  ))}
